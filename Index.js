...
document.querySelector('.CurrencyFrom').innerHTML = option1
...
document.querySelector('.CurrencyTo').innerHTML = option2

// Onchange Option 1
function FromCurrency(e) {
    console.log(e.value);
    option_1_Value = e.value;
}

// Onchange option 2
function ToCurrency(e) {
    option_2_Value = e.value;
}


let MyCurrencyExchange = async () => {
    ...
    let data = await fetch(`https://api.apilayer.com/fixer/convert?to=${option_2_Value}&from=${option_1_Value}&amount=${amount.value}`, requestOptions);
    ...
    mydata.map((elem) => {
        ...
        html =
            `
    <div class="card col-md-6" style="padding: 10px; border-radius: 10px;">
    <div class=" text-center">
    <h5 class="card-header">Rate Information</h5>
    <p >${Math.round(elem.info.rate)} ${option_1_Value} </p>
    </div>
    
    <div class=" text-center">
    <h5 class="card-header">Information</h5>
    
    <p >You Change From <span class="" >${elem.query.from}</span>   <span >To <span class="">${elem.query.to}</span> </span> </p>
 
    <p>Amount You Entered = <span>${Math.round(elem.query.amount)} ${option_1_Value}</span></p>
    <p>Total Amount = <span>${Math.round(elem.result)} ${option_1_Value}</span></p>
    </div>
    </div>
    ...
    let img = `  <img src="./imgs/spiner.gif" class="mySpinner"  style="width:48px;height:48px;"></img>`
    ...
    document.querySelector('.all_data').innerHTML = img;
    setTimeout(() => {
        document.querySelector('.all_data').innerHTML = html;
        //  document.querySelector('.chart').innerHTML = html2
    }, 1000);
